 ...
 * this story owns (admin, courses, knowledge-base, setter, onboarding).
 *
 ...
 * the same mapping under the directory it owns
 * (app/api/dashboard/_lib/http.ts). This file deliberately mirrors that one's
 * exported names, status mapping and response body verbatim, so the two
 * collapse into a single `lib/api/route-guard.ts` without either caller
 * changing when one owner exists for that path.
 *
 ...
 * prevent, and the reason the setter board can tell "no leads" from "the
 * refresh failed".
 */
